...
onready var tileNames = GameManager.TILES.keys()
...
func _input(event):
	...
	if Input.is_mouse_button_pressed(BUTTON_WHEEL_UP):
		selectedTile -= 1
		if selectedTile < 0: selectedTile = tileNames.size()-1
		changeTileSp(selectedTile)
	elif Input.is_mouse_button_pressed(BUTTON_WHEEL_DOWN):
		selectedTile += 1
		if selectedTile >= tileNames.size(): selectedTile = 0
		changeTileSp(selectedTile)
	pass
...
func changeTileSp(tileIndex):
	var tex = load(str("res://Sprites/GroundTiles/", tileNames[tileIndex],".png"))
