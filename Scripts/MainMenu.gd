...
func createWorldFile():
	...
	if !worldFolderDir.dir_exists("user://TerrainGenerationEngine2"):
		worldFolderDir.open("user://")
		worldFolderDir.make_dir("TerrainGenerationEngine2")
	
	#criando pasta "Worlds" caso ela não exista 
	if !worldFolderDir.dir_exists("user://TerrainGenerationEngine2/Worlds"):
		worldFolderDir.open("user://TerrainGenerationEngine2")
	...
	data.open(str("user://TerrainGenerationEngine2/Worlds/",worldNameLine.text,".dat"),File.WRITE)
